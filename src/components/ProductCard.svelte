...
<div class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 lg:gap-20 relative text-base">
		...
		<div class="flex flex-col gap-3">
			{#each productFeature.featureList as listItem}
				<div class="flex gap-2 items-center">
					<div
						class="grid place-items-center px-1.5 text-xs sm:text-sm aspect-sqaure rounded-full border-[1.5px] bg-white border-solid border-green-300"
					>
	...
	<div
		class={'flex flex-col dropShadow overflow-hidden rounded-b-lg ' +
			(index % 2 === 1 ? 'order-1' : '')}
	>
		<div class="rounded-t-xl sm:h-10 bg-white opacity-60 px-3 flex items-center gap-2">
			{#each [0, 1, 2] as i}
